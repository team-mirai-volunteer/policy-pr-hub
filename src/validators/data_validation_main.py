...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="GitHub APIとローカルデータの整合性を検証するスクリプト"
    )
...
def main():
    ...
    try:
        ...
        result = validator.validate_data(
            data_dir=args.data_dir, output_file=output_file
        )
        ...
        if args.verbose:
            ...
            for state, counts in result["comparison"]["differences"]["states"].items():
                print(
                    f"  {state}: GitHub={counts['github']:,}, ローカル={counts['local']:,}, 差異={counts['difference']:+,}"
                )

            if result["comparison"]["differences"]["labels"]:
                print("\nラベル別比較（上位5件）:")
                for i, (label, counts) in enumerate(
                    list(result["comparison"]["differences"]["labels"].items())[:5], 1
                ):
                    print(
                        f"  {i}. {label}: GitHub={counts['github']:,}, ローカル={counts['local']:,}, 差異={counts['difference']:+,}"
                    )
